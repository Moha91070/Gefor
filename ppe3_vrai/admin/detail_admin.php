...
<h2>Dossier candidature - Formation continue</h2>
    ...
    <fieldset>
       <legend>Pré selection</legend>
       <a href="sql/sql_pre_selc.php?choix=7&id=<?=$id?>" onclick="return confirm('Confirmation acceptation <?=$resultat['nom_dusage'] ?> <?=$resultat['prenom'] ?>')"><button>Accepter</button></a> 
       <a href="sql/sql_pre_selc.php?choix=6&id=<?=$id?>" onclick="return confirm('Confirmation acceptation <?=$resultat['nom_dusage'] ?> <?=$resultat['prenom'] ?>')"><button>Refuser</button></a> 
    </fieldset>
</body>       
</html>